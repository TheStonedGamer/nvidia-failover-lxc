...
| `PROXY_GUARD_DEGENERATE` | `1` | fail over when a model degenerates — `finish_reason=repetition` or a detected **repetition loop** (a short phrase repeated over and over). Non-stream fails over; streaming is truncated cleanly and the model is cooled so the sticky cursor rolls off it |
